...
const Tabs = ({ config = {} }: { config: any }) => {
  ...
  const tabContentEl: React.LegacyRef<HTMLDivElement> = useRef(null);
  ...
  useEffect(() => {
    const node: any = tabContentEl.current;
    var panes = _.map(
      node.children,
      (child) => child.querySelector(".is-invalid") !== null
    );
          ...
          <div className={tabColumnClass}>
            <nav>
              <div id={tabId} className={tabListClass}>
                {_.map(tabs, (tab, key) => {
                  const tabInvalid =
                    (tabValidations as any).next().value === true;
                  return (
                    <a
                      key={key}
                      href={`/tab${tabId}`}
                ...
                (
                  ...
                  <div
                    ...
                    className={`${tabPaneClass} ${
                      activeTab === tabKey ? tabActiveClass : ""
                    }`}
                  >
                    {comment && (
                      <small className={commentClass}>{comment}</small>
                    )}
