...
const onSortEnd = (move: Function, { oldIndex, newIndex }: PointerArgs) =>
  move(oldIndex, newIndex);
const SortableItem = SortableElement((props: TableRowProps) =>
  renderTableRow(props)
);
const SortableTableBody = SortableContainer((props: TableBodyProps) =>
...
const renderTableRow = ({
  ...
  <tr key={rowIndex}>
    ...
    {_.map(elements, ({ name, label, ...rest }, key) => (
      <td key={key}>
        <Element
          config={{ ...rest, name: joinNames(fieldArrayName, rowIndex, name) }}
        />
      </td>
    ))}
        ...
        {!!buttons.remove &&
          (_.isFunction(buttons.remove) ? (
            ...
            <button
              ...
              onClick={() => arrayActions.remove(rowIndex)}
            >
              {buttons.remove}
            </button>
          ))}
        {!!buttons.duplicate &&
          (_.isFunction(buttons.duplicate) ? (
            ...
            <button
              ...
              onClick={() => arrayActions.push(value)}
            >
              {buttons.duplicate}
            </button>
          ))}
...
const EditableGrid = ({
  ...
  const tableWidth =
    _.map(elements, "width").reduce((sum, num) => sum + num, 50) || "100%";
            ...
            <table className={tableClass} style={{ width: tableWidth }}>
              <thead>
                <tr>
                  ...
                  {isObject === false && !!buttons && !!buttons.remove && (
                    <th></th>
                  )}
                </tr>
              </thead>
                    ...
                    <td colSpan={_.size(elements) + additionalColumnCount}>
                      {_.isFunction(buttons.add) ? (
                        ...
                        <button
                          ...
                          onClick={() => arrayActions.push(arrayFields)}
                        >
                          {buttons.add}
                        </button>
                      )}
