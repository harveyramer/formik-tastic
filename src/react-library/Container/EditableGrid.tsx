...
const renderTableBody = ({ isObject = false, isSortable, hasValue, arrayValues, ...rowProps }:TableBodyProps) => (
    <tbody>
        { hasValue ? _.map(arrayValues, ( value, index ) =>
            isObject === false && isSortable
                ? <SortableItem key={ index } index={ index } isSortable={ isSortable } { ...rowProps } />
                : renderTableRow({ ...rowProps, index, rowIndex: index, value, isObject })
        ) : null }
    </tbody>
);

const renderTableRow = ({ fieldArrayName, elements, arrayActions, rowIndex, buttons, isSortable, value = {}, isObject = false }:TableRowProps) => (
    <tr key={ rowIndex }>
...
const renderSortableHandle = ( ) => <td><i className="fas fa-grip-vertical"></i></td>;
